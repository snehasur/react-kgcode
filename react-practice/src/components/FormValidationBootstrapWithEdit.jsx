...
const FormValidationBootstrapWithEdit = () => {
  ...
  const handleEdit = (id) => {
    const entryToEdit = dataList.find((item) => item.id === id);
    if (entryToEdit) {
      setInputValueFname(entryToEdit.fname);
      setInputValueLname(entryToEdit.lname);
      setInputValueEmail(entryToEdit.email);
      setInputValuePhone(entryToEdit.phone);
      setInputValueCity(entryToEdit.city);
      setInputValueState(entryToEdit.state);
      setInputValueZip(entryToEdit.zip);
      setInputValueAddress(entryToEdit.address);
      setInputValueGender(entryToEdit.gender);
      setInputValueDOB(entryToEdit.dob);
      setIsAgreed(entryToEdit.agreed);
      setIsEditing(true);
      setCurrentId(id);
    }
  };
